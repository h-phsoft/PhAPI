...
class PgSqlBuilder {
  ...
  buildSelect(entity, options = {}) {
    ...
    for (const [key, value] of Object.entries(filters)) {
      ...
      if (fieldMeta && fieldMeta.query) {
        whereClauses.push(`"${fieldMeta.Name}" = $${paramIndex++}`);
        params.push(value);
      }
    }
  ...
  buildInsert(entity, data) {
    ...
    const sql = `INSERT INTO "${tableName}" (${columns.join(', ')}) VALUES (${placeholders.join(', ')}) RETURNING "${pkCol}"`;
    return { sql, params };
  }
  ...
  buildUpdate(entity, id, data) {
    ...
    for (const fieldMeta of entity.fields) {
      ...
      if (fieldMeta.update && data.hasOwnProperty(apiField)) {
        setClauses.push(`"${fieldMeta.Name}" = $${paramIndex++}`);
        params.push(data[apiField]);
      }
    }

    params.push(id);
    const sql = `UPDATE "${tableName}" SET ${setClauses.join(', ')} WHERE "${pkCol}" = $${paramIndex++}`;
    return { sql, params };
  }
  ...
  buildDelete(entity, id) {
    ...
    const sql = `DELETE FROM "${tableName}" WHERE "${pkCol}" = $1`;
