...
class UnifiedRepository {
   ...
   * shaped as find() shapes a page and handed to `onBatch`. Memory holds one
   * batch, whatever the result's size. The caller decides the ceiling, because
   * only it knows what the rows are for.
   *
   * @param {Object} entity
   * @param {Object} options As find(); page and pageSize are ignored
   * @param {Object} context
   * @param {number} limit The most rows the statement may return
   * @param {function(Array<Object>): (boolean|void|Promise<boolean|void>)} onBatch
   *   Called with each batch, and waited for; false stops the read
   * @returns {Promise<void>} Settles once the cursor is released
   */
  async stream(entity, options, context, limit, onBatch) {
    ...
    await poolWrapper.stream(sql, params, (rows) => onBatch(shapeDates(entity, this.mapToCamelCase(rows, entity))));
  }
